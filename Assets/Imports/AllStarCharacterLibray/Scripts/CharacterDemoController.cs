...
public class CharacterDemoController : MonoBehaviour
{
	...
	//unarmed, 1H, 2H, bow, dual, pistol, rifle, spear and ss(sword and shield)
		...
		*/
		if (!Input.GetKey (KeyCode.LeftAlt)) { // if we're changing camera transforms, do not use "USE"
			if (Input.GetMouseButton (1)) {// are we using the right button?
				if (rightButtonDown != true) {// was it previously down? if so we are already using "USE" bailout (we don't want to repeat attacks 800 times a second...just once per press please// RUNGY addin enemy collider "test for nearest hit" here, they need to actually take precednce over the ground
